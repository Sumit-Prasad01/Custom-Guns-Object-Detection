...
class FasterRCNNModel:
    ...
    def create_model(self):
        try:
            ...
            logger.info(f"Failed to create model {e}")
            raise CustomException(f"Failed to create model {e}")
    ...
    def compile(self, lr = 1e-4):
        try:
            ...
            logger.info(f"Failed to compile model {e}")
            raise CustomException(f"Failed to compile model {e}")
        ...
        try:
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                for images, targets in tqdm(train_loader, desc = f"Epoch : {epoch}"):
                    images = [img.to(self.devive) for img in images]
                ...
                logger.info(f"Epochs completed with train loss {total_loss}")
